...
func getVersion() (string, error) {
    ...
    // Convert dashes to tildes for prerelease versions
...
func checkGreatest(version string) (string, error) {
    ...
    if len(tags) == 0 {
        return fmt.Sprintf("Version %s is the greatest (no other tags found)", version), nil
    }
    ...
    for _, tag := range tags {
        ...
        if CompareVersions(tagVer, currentVer) > 0 {
            return "", fmt.Errorf("version %s is not the greatest among all tags", version)
        }
    }

    return fmt.Sprintf("Version %s is the greatest among all tags", version), nil
}