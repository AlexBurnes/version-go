...
// Version types based on semantic versioning
...
// checkVersionType determines the type of a version string using semver
func checkVersionType(version string) VersionType {
	v, err := semver.NewVersion(version)
	if err != nil {
		return VersionTypeInvalid
	}

	// If has prerelease, it's either prerelease or postrelease
	if v.Prerelease() != "" {
		pre := v.Prerelease()
		if strings.HasPrefix(pre, "alpha") || strings.HasPrefix(pre, "beta") || 
		   strings.HasPrefix(pre, "rc") || strings.HasPrefix(pre, "pre") {
			return VersionTypePrerelease
		}
		if strings.HasPrefix(pre, "fix") || strings.HasPrefix(pre, "post") || 
		   strings.HasPrefix(pre, "next") {
			return VersionTypePostrelease
		}
		return VersionTypeIntermediate
	}

	return VersionTypeRelease
}

// checkVersion checks if a version string is valid according to semver
...
// getVersionTypeStr returns the version type as a string for a given version
func getVersionTypeStr(version string) (string, error) {
	if err := checkVersion(version); err != nil {
		return "", err
	}
	return checkVersionType(version).String(), nil
}

// getBuildType returns the CMake build type for a given version
func getBuildType(version string) (string, error) {
	if err := checkVersion(version); err != nil {
		return "", err
	}
	return checkVersionType(version).BuildType(), nil
}
...
func checkGreatest(version string) (bool, error) {
	...
	tags, err := getGitTags()
	if err != nil {
		return false, fmt.Errorf("failed to get git tags: %v", err)
	}

	if len(tags) == 0 {
		return true, nil // If no tags exist, current version is greatest
	}
...
Usage:
    version [-h|--help] [-v|--version] [-r|--release] command [version]
...
Commands:
    ...
    check [ver]     check if version is valid (uses current git version if not specified)
    check-greatest [ver] check if version is the greatest among all tags (uses current git version if not specified)
    type [ver]      show semantic version type (uses current git version if not specified)
    build-type [ver] show CMake build type (Release/Debug) based on version type
`, VERSION)
}
...
func main() {
	...
	// Get version from args or git if needed
	getVersionArg := func() (string, error) {
		if len(args) > 1 {
			return args[1], nil
		}
		return getVersion()
	...
	case "check":
		...
		if err == nil {
			result = fmt.Sprintf("Version %s is valid", version)
		}
	case "check-greatest":
		version, e := getVersionArg()
		if e != nil {
		...
		isGreatest, e := checkGreatest(version)
		if e != nil {
			err = e
			break
		}
		if isGreatest {
			result = fmt.Sprintf("Version %s is the greatest among all tags", version)
		} else {
			err = fmt.Errorf("version %s is not the greatest among all tags", version)
		}
	case "type":
		version, e := getVersionArg()
		if e != nil {
		...
		result, err = getVersionTypeStr(version)
	case "build-type":
		version, e := getVersionArg()
		if e != nil {
			err = e
